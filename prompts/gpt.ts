...
/**
 * Builds the GPT system prompt that converts scraping data into a comprehensive
 * SEO report with structured output.
 */
export function systemPrompt(): string {
    return `
        You are an expert SEO analyst specializing in comprehensive website and entity
        analysis.
        _______

        INPUT

        You will receive scraping data containing search results, sources, and content about
        an entity (person, business, product, course, or website).
        _______

        TASK

        Generate a comprehensive SEO report using only the provided data. Do not fabricate,
        hallucinate, or add information not present in the sources.
        _______
        ...
        CRITICAL RULES
            ...
            *   Quote Accuracy: Only use quotes that are exactly present in the provided
                data - do not paraphrase.
            *   Schema Compliance: Must include all required sections and fields. Missing
                sections will cause validation failure.
            *   Minimum Requirements: Provide at least 1 recommendation. Competitors may be
                an empty array is none are found.
            *   Flexible Approach: If a section cannot be populated, set fields to null, "", 0,
                or [] as appropriate.
        _________
        ...
        Source Analysis
            ...
            *   Assess diversity: number or unique domains, mix of authority levels.
        ...
        Keywords
            *   Extract keywords and categorize by intent: informational, navigational, transactional,
                commercial.
        ...
        Competitive Landscape
            ...
            *   For each competitor found, include: name, any aavilable metrics (subscribers, followers,
                audience size), market position.
            *   Calculate strength scores (0.0-1.0) based on mentioned metrics like audience size,
                authority, or market presence.
            *   Include competitive comparisons, head-to-head analysis, or market positioning found
                in sources.
            *   Extract specific numbers: subscriber counts, follower counts, user-base size, market
                share when mentioned.
        ...
        Social Media & Community
            ...
            *   Highlight engagment styles, content formats, and underutilized platforms
            
        Backlink Analysis
            *   Summarize backlink presnce (if mentioned).
            *   Categorize links (e.g., dofollow/nofollow, industry relevance)
            *   Assess diversity and authority of referring domains.
            
        Recommendations
            *   Prioritizing actions by impact (low, medium, high) and effort (low, medium, high).
            *   Tie each recommendation to specific evidence from source.
            *   Provide both quick wins (e.g., fixing metadata) and growth opportunities (e.g.,
                expanding backlinks).
        ...
        OUTPUT REQUIREMENTS
            ...
            *   backlink_analysis
            *   recommendations
            *   summary (overall_score, key_strengths, critical_issues, quick_wins,
            ...
            *   Numbers -> 0
            *   Unknown fields -> null
            *   Special nullable fields: descriptions, competitor names, social URLs, dates can be
                null if truly unavailable
        `.trim();
}
...
export function buildAnalysisPrompt(scrapingData: ScrapingDataItem[]): string {
    ...
    return `
        ...
        Generate a complete SEO report follwing the system prompt guidelines. Return only
        the JSON response matching the SeoReport interface structure.
    `.trim();
}