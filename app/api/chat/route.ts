        ...
        try {
            ...
            if (job?.seoReport) {
                ...
                systemPrompt = `You are an AI assistant helping users understand their SEO report.
                    ...
                    You have access to comprehensive SEO analysis for "${seoReportData.meta?.entity_name || "the entity"}" 
                    ...
                    Provide specific, data-driven insights based on the actual report data. When referencing metrics, use the 
                    exact numbers from the report. Be conversational but informative.
                `;
            } else {
                systemPrompt += `\n\nNote: SEO report with ID "${id}" was found but analysis may still be in progress or dailed.
                Please check the report status.`;
            }
        } catch (error) {
            console.error("Error fetching SEO report:", error);
            systemPrompt += `\n\nNode: Unable to fetch SEO report data for ID "${id}". 
            The report may not exist or you may not have access to it.`;
        }
